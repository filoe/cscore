...
{
    /// <summary>
    /// The IXAudio2VoiceCallback interface contains methods that notify the client when certain events happen in a given <see cref="XAudio2SourceVoice"/>.
    /// </summary>
    [ComVisible(true)]
    ...
    public interface IXAudio2VoiceCallback
    {
        ...
        /// Note: In a situation where there is always plenty of data available on the source voice, BytesRequired should always report zero, because it doesn't need any samples immediately to avoid glitching.</param>
        ...
        /// </summary>
        /// <param name="bufferContextPtr">Context pointer that was assigned to the pContext member of the <see cref="XAudio2Buffer"/> structure when the buffer was submitted.</param>
